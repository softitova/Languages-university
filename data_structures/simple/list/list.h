...
public:
    struct iterator
    {
        friend struct iterator_list;
        iterator (iterator_list* obj, const std::string &key);
        iterator (node& other, iterator_list* obj);
        iterator (const std::string &key, const iterator &other);
        iterator (const std::string &key, const iterator &prev, const iterator &next, iterator_list* obj);
    ...
    private:
        node *cur;
        iterator_list *my_object;
        bool flag;
    };

    struct const_iterator
    {
        const_iterator (const iterator_list* obj, const std::string &key);
        const_iterator (node& other, const iterator_list* obj);
        const_iterator (const std::string &key, const const_iterator &other);
        const_iterator (const std::string &key, const const_iterator &prev, const const_iterator &next, const iterator_list* obj);
    ...
    private:
        node *cur;
        const iterator_list* my_object;
        bool flag;
    };
    iterator_list();

    iterator_list(const iterator_list &my_list);

    ~iterator_list();

    iterator_list & operator = (const iterator_list & s);
    ...
    void splice(iterator pos, iterator_list & other, iterator first, iterator last);

    void swap(iterator_list &b);

    void clear();

private:

    mutable std::vector<iterator*> v;

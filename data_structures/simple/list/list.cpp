...
typename list::iterator list::iterator::operator=(iterator other)
{
    this->swap(other);
    return *this;
}
...
typename list::const_iterator list::const_iterator::operator=(const_iterator other)
{
    this->cur = other.cur;
    return *this;
}
...
typename list::iterator list::insert(iterator pr, std::string const &s)
{
    ...
    {
        iterator temp(s, iterator(this, std::string()), pr, this);
...
typename list::iterator list::erase(iterator pos)
{
...
//    exit(0);
    ...
    iterator y = last;
    iterator x = first;
    first == other.begin() ? x = iterator(&other, std::string("")) : x--;
    last--;
    x.cur->next = y.cur;
    y.cur->prev = x.cur;
    if (empty())

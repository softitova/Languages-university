...
template<typename Func_t,typename ... Params>
struct my_bind {
    my_bind(Func_t&& f, const Params&& ... params) :
    f(std::forward<Func_t>(f)), p(std::forward<Params>(params) ... ) {}
...
private:
    //typedef  typename std::decay<Func>::type Func_t;
    Func_t f;
...
{
    return my_bind<Func_t,  Params ...>(std::forward<Func_t>(f), params...);
}
